function [state_hat] = Localization_RPF2(f,h,z,control_commands)
...
if isempty(firstRun)
    ...
    x_hat_particle = ones(dim_state, N_particle,'gpuArray');   % 각 파티클의 추정값
...
%% RPF
X = x_hat_particle;
new_X = zeros(dim_state,N_particle);
temp_q = zeros(1,N_particle);
control_lin = control_commands(1);
control_ang = control_commands(2);
parfor i = 1 : N_particle
...
%     end
    ...
    new_X(:,i) = double(f(X(1,i),X(2,i),X(3,i),control_lin,control_ang))' + noise_w;
    z_hat = double(h(X(1,i),X(2,i),X(3,i),control_lin,control_ang))' + noise_v;

    error = z - z_hat;
    temp_q(i)=1 / ( (2*pi)^(dim_measurement/2)*det(R)^(1/2) )*exp( -error'/inv(R)*error/2 );
end
x_hat_particle = new_X;
q = temp_q;
